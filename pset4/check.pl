...
sub boolenv ($) {
    my($e) = @_;
    return nonemptyenv($e) && $ENV{$e} ne "0";
}
my($NOSTDIO) = boolenv("NOSTDIO");
my($NOYOURCODE) = boolenv("NOYOURCODE");
my($TRIALTIME) = nonemptyenv("TRIALTIME") ? $ENV{"TRIALTIME"} + 0 : 3;
my($STDIOTRIALTIME) = nonemptyenv("STDIOTRIALTIME") ? $ENV{"STDIOTRIALTIME"} + 0 : $TRIALTIME;
my($TRIALS) = nonemptyenv("TRIALS") ? int($ENV{"TRIALS"}) : 5;
$TRIALS = 5 if $TRIALS <= 0;
my($STDIOTRIALS) = nonemptyenv("STDIOTRIALS") ? int($ENV{"STDIOTRIALS"}) : $TRIALS;
$STDIOTRIALS = $TRIALS if $STDIOTRIALS <= 0;
my($MAXTIME) = nonemptyenv("MAXTIME") ? $ENV{"MAXTIME"} + 0 : 20;
$MAXTIME = 20 if $MAXTIME <= 0;
my($MAKETRIALLOG) = boolenv("MAKETRIALLOG");
...
my($VERBOSE) = boolenv("VERBOSE");
my($NOMAKE) = boolenv("NOMAKE");
my($STRACE) = boolenv("STRACE");
$NOSTDIO = 1 if $STRACE;
...
sub run_sh61 ($;%) {
    ...
    if (exists($answer->{"status"})
        ...
        my($sigid) = $answer->{"status"} & 255;
        my($signame) = $signames[$sigid];
        $signame = defined($signame) ? "$signame signal" : "signal $sigid";
...
sub run_qitem ($) {
    ...
    my($trialtime) = $qitem->{"type"} eq "stdio" ? $STDIOTRIALTIME : $TRIALTIME;
    if ($qitem->{"errors"} > 1) {
        $nprev = $qitem->{"count"};
    ...
    for (my $qpos = 0; $qpos < @workq; $qpos += 1) {
        ...
        if ($type ne $qitem->{"type"}) {
            print "STRACE COMMAND:\n           ", $qitem->{"command"}, "\n"
                if !exists($ENV{"NOCOMMAND"}) && $STRACE;
            $type = $qitem->{"type"};
        ...
        if ($stdiot
            ...
            my($color);
            if ($ratio < 0.75 && $number != 1 && $number != 2 && $number != 6) {
                $color = $Redctx;
            } elsif ($ratio < 1.9) {
                $color = $Cyan;
            } else {
