...
simple_printer& simple_printer::snprintf(const char* fmt, ...) {
    va_list val;
    va_start(val, fmt);
    size_t n = vsnprintf(s_, end_ - s_, fmt, val);
    va_end(val);
    s_ += n;
    return *this;
}

simple_printer& simple_printer::operator<<(spflush flush) {
    if (s_ != buf_) {
        ssize_t nw = write(flush.fd_, buf_, s_ - buf_);
        assert(nw == s_ - buf_);
        s_ = buf_;
    }
    return *this;
