...
my($info) = run_sh61("./breakout61 -B0 -d0.01 -w13 -h8 -b6 -s4 -p0.05", "stdin" => "/dev/null", "stdout" => "pipe", "time_limit" => 0.6, "size_limit" => 10000);
...
$info = run_sh61("./breakout61 -B0 -d0.01 -w26 -h16 -b10 -s20", "stdin" => "/dev/null", "stdout" => "pipe", "time_limit" => 1, "size_limit" => 10000);
if (!exists($info->{"killed"}) || $info->{"killed"} !~ /^timeout/) {
    print OUT "${Red}FAILURE${Redctx} (expected timeout, got ",
        (exists($info->{"killed"}) ? $info->{"killed"} : "exit status " . $info->{"status"}),
        ")${Off}\n";
...
$info = run_sh61("./breakout61", "stdin" => "/dev/null", "stdout" => "pipe", "time_limit" => 3, "size_limit" => 10000);
if (!exists($info->{"killed"}) || $info->{"killed"} !~ /^timeout/) {
    print OUT "${Red}FAILURE${Redctx} (expected timeout, got ",
        (exists($info->{"killed"}) ? $info->{"killed"} : "exit status " . $info->{"status"}),
        ")${Off}\n";
