...
# Optimization level 2 and no position-independent executables by default
O ?= 2
PIE ?= 0
...
test%: m61.o basealloc.o hexdump.o test%.o
	$(call run,$(CXX) $(CXXFLAGS) $(LDFLAGS) $(O) -o $@ $^ $(LIBS),LINK $@)

hhtest: m61.o basealloc.o hexdump.o hhtest.o
	$(call run,$(CXX) $(CXXFLAGS) $(LDFLAGS) $(O) -o $@ $^ $(LIBS),LINK $@)
